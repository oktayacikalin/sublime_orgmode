...
* Features
  ...
  * Context sensitive actions
    ...
    - External link opener on pressing enter
      ...
      - Plugin: TestRail
        - Normal call: [[testrail:1234]]
        - Aliases test and t: [[test:1234]]
      - Plugin: eMail
...
* Installation
  ...
  * Key bindings for interaction
    See the file [[Default (OSX).sublime-keymap]] for details. Linux and Windows are currently missing. If you are on such a system please send my your best shot on the keymap bindings - thanks!
...
* Todo [1/21]
  ...
  - [X] Copy on shell context without selection should copy everything after the "$ " until the eol into the clipboard.
  - [ ] Copy on table context without selection should copy the current table row as tab separated values into the clipboard.
  - [ ] Press tab on external link to grab page title and replace rest of line with it.

* Known Issues
  - Keymap bindings for Linux and Windows are missing.
